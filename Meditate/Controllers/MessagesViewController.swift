...
//        loadMessages()
    ...
    func setPeer(uid: String) {
        self.uidString = uid
        loadMessages(uid: uid)
    }
    
    //MARK: - Networking
    
    private func loadMessages(uid: String) {
        let ref = Database.database().reference().child("chat").child(uid)
    ...
    @IBAction func sendButtonPressed(_ sender: Any) {
        ...
        Database.database().reference().child("chat").child(self.uidString).child(id).setValue(regObject)
